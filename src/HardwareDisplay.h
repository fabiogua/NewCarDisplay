...
public:
    ...
    void setImdOk(uint8_t value)
    {
        _imd = value;
        if (_imd)
            lv_label_set_text(ui_Label5, "IMD OK");
        else
            lv_label_set_text(ui_Label5, "IMD ERROR");
    }

    void setBmsOk(uint8_t value)
    {
        _bms = value;
        if (_bms)
            lv_label_set_text(ui_Label7, "BMS OK");
        else
            lv_label_set_text(ui_Label7, "BMS ERROR");
    }
    ...
    message_parse_result_t parseMessage(CarMessage &carMessage)
    {
        ...
        for (car_sub_message_t &subMessage : carMessage.subMessages)
        {
            ...
            {
                setBmsOk(subMessage.data[1]);
                setImdOk(subMessage.data[2]);
